...
D1 (SQLite) 上の12テーブル。Drizzle でスキーマを定義し、`drizzle-kit` が生成した SQL を `wrangler d1 migrations apply` で適用する。
...
| body | text | not null, 1000文字以内 | 改行を保持して表示 |
| url | text | null, URL検証あり | 関連する資料などへの任意リンク |
| created_at / updated_at | text | not null | UTC ISO 8601 |

索引: `(concert_id, created_at)`。1演奏会につき最大10件とし、アプリ層で登録時に上限を検証する。表示順は `created_at DESC, id DESC` とし、編集しても掲載順を変えない。
...
`concerts` を削除すると配下の `practices`・`practice_media`・`pieces`・`concert_resources`・`announcements` も削除される。`venues` は削除しても練習は残り、会場が未設定になる（過去の練習記録を失わないため）。未使用の `link_checks` にデータは作成しない。
...
| `/` | 全 | ダッシュボード。次の練習を最上位に置き、最新のお知らせ、本番情報・出欠・備考・資料・問い合わせへ続ける |
| `/practices` | 全 | 練習日程一覧。「今後」「過去」を切り替え。各練習に会場・詳細・録音・地図・カレンダー導線 |
| `/pieces` | 全 | 曲一覧とボウイングリンク |
| `/admin` | admin | `/admin/concerts` へ誘導。管理内の切替はサブナビ（演奏会・練習・曲・お知らせ・会場・設定）のみ |
...
| `/admin/announcements` | admin | 選択中の演奏会のお知らせを作成・編集・削除 |
...
- 色はクールなニュートラル背景、インク文字、深いボルドー（CTA・アクティブ）を基本とし、ダークテーマでもコントラストを保つ
- 本文は Noto Sans JP（ゴシック）、見出しとホームの日付は Noto Serif JP（明朝）
...
管理者は `/admin/announcements` から、選択中の演奏会にタイトル・本文・任意の外部URLを持つお知らせを作成・編集・削除できる。作成日時を掲載日時として扱い、新しい順に表示する。編集では掲載順を変えない。管理画面で演奏会を切り替えたときは、別の演奏会へ入力値を持ち越さないようフォームを初期化する。保存に失敗した場合は入力内容を残してエラーを表示し、削除には既存の確認ダイアログを使う。
...
- 演奏会の選択解決順序
- 演奏会資料の5件上限と並べ替え
- お知らせの10件上限、新しい順、演奏会ごとの分離、作成・編集・削除
...
1. エキストラのパスワードでログインし、演奏会情報・お知らせ・練習日程・資料・問い合わせ導線・曲とボウイングが見えることを確認する
2. 管理者のパスワードでログインし、会場・演奏会・資料・練習・お知らせを作成し、練習を複製して、閲覧側に反映されることを確認する
...
- お知らせの固定表示、予約投稿、個別の既読管理、団体共通のお知らせ、外部通知
- パスワードハッシュのより強い方式への移行（形式にバージョン識別子があるため段階移行できる）

### 受け入れで見つかった UI 改善（Phase 8）
...
> **この表は履歴です。生きている ADR 索引の正は [platform/design.md の決定記録](../../platform/design.md) です。ここへ行を足さないでください。**

実装中に行った判断は `docs/adr/` に ADR として1件1ファイルで記録する。書き方と運用は [ADR-0000](../../adr/0000-use-markdown-architectural-decision-records.md) が正。以下はアーカイブ時点（ADR-0022 まで）のスナップショットである。
