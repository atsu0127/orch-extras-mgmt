...
ダッシュボード（ホーム）は演奏会プログラムに近い**パンフレット配置**とする。**次の練習**を中央寄せの大きな日付（明朝）で最上段に置き、その直後に本番・出欠・備考・資料・問い合わせを hairline 区切りで続ける（情報順は[ADR-0017](./adr/0017-departure-board-dashboard-layout.md)）。練習日程・曲・管理などの一覧・操作画面は**実用コンパクト**（平坦パネルと行リスト）のままとする（[ADR-0018](./adr/0018-compact-utility-visual-language.md)）。主要画面への移動は下部固定ナビ（アイコン＋短いラベル: ホーム / 練習 / 曲。管理者は管理を追加）とし、演奏会セレクタはヘッダに常時置く（[ADR-0015](./adr/0015-adopt-mantine-with-purpose-built-theme.md)）。
