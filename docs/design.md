...
ダッシュボードは「実用コンパクト」の実用アプリを基準にする。**次の練習**を第一画面の要約バー（日付・時刻・会場・地図）として最上段に置き、その直後に本番情報と出欠回答を配置し、備考・資料・問い合わせは続ける（情報順は[ADR-0017](./adr/0017-departure-board-dashboard-layout.md)、見た目は[ADR-0018](./adr/0018-compact-utility-visual-language.md)）。主要画面への移動は下部固定ナビ（アイコン＋短いラベル: ホーム / 練習 / 曲。管理者は管理を追加）とし、演奏会セレクタはヘッダに常時置く（[ADR-0015](./adr/0015-adopt-mantine-with-purpose-built-theme.md)）。
...
- すべてを同じ角丸カードに入れず、日程や資料は平坦なパネルと行リストを基本にする
...
- タイポは読みやすいゴシック一本（IBM Plex Sans JP）。装飾用の明朝は使わない
...
| 閲覧 UI は実用コンパクト、下部ナビはアイコン＋短いラベル | 密度と走査性を揃え、タブ識別を速くする | [ADR-0018](./adr/0018-compact-utility-visual-language.md) |
